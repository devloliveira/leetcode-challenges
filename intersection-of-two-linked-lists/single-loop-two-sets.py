...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        visited_ids_second_list = set()

        get_node_id = lambda node: id( node )
        iter_node_1 = headA
        iter_node_2 = headB

        while (iter_node_1 is not None) or (iter_node_2 is not None):

            if iter_node_1:
                node_1_id = get_node_id( iter_node_1 )
                if node_1_id in visited_ids_second_list:
                    return iter_node_1
                visited_ids_first_list.add( node_1_id )
                iter_node_1 = iter_node_1.next

            if iter_node_2:
                node_2_id = get_node_id( iter_node_2 )
                if node_2_id in visited_ids_first_list:
                    return iter_node_2
                visited_ids_second_list.add( node_2_id )
